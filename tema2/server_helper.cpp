...
void handle_connection_request(int sock_tcp, int &fd_max, fd_set &read_fds,
	...
	if (all_clients.find(new_client_id) != all_clients.end()) { 
		reconnect(new_client_id, newsockfd, cli_addr, active_clients, 
				  all_clients, active_ids_sockets, store_and_forward);
		return;
	}
	
	// daca clientul este nou
	add_new_client(newsockfd, new_client_id, cli_addr, active_clients, 
				   all_clients, active_ids_sockets);
}
...
static void send_to_subscribers(subscription *s, udp_message message, sockaddr_in from_station,
								unordered_map<string, int> active_ids_sockets,
								unordered_map<string, list<sub_message>> &store_and_forward) {
...
void handle_udp_message (char buffer[], int sock_udp, int i,
						sockaddr_in from_station, fd_set &read_fds,
						unordered_map<string, int> active_ids_sockets,
	...
	for (auto s = it->second.begin(); s != it->second.end(); s++) {
		send_to_subscribers(&(*s), message, from_station, active_ids_sockets, store_and_forward);
	}
}
...
void handle_tcp_message(int i, fd_set &read_fds,
	...
	if (req.req_type == SUBSCRIBE) {
		// daca clientul e deja in lista, ii actualizez sf-ul
		for (auto s = it->second.begin(); s != it->second.end(); s++) {
