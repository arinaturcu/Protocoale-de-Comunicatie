...
void ping(int sockfd, struct in_addr ip_addr, int count)
{
	...
	if (count == -1) count = (unsigned short) count;
	...
	for (int i = 0; i < count; ++i) {
		...
		*/
		...
		printf("%d bytes from %s: icmp_seq=%d ttl=%d time=%lf ms\n", ntohs(rip_hdr->tot_len), ip_s, ntohs(ricmp_hdr->un.echo.sequence), rip_hdr->ttl, time_elapsed);
	}
}
...
void traceroute(int sockfd, struct in_addr ip_addr)
{
	 ...
	 * funcționalitatea implementată în funcția de ping. */
		...
		icmp_hdr->un.echo.sequence = htons(counter);
		icmp_hdr->checksum = 0;
		icmp_hdr->checksum = checksum(icmp_hdr, (sizeof(struct icmphdr)));
		...
		printf("%d \t %s\t %lf ms\n", counter, inet_ntoa(rip_addr), time_elapsed);
		counter++;

		if (ricmp_hdr->type == ICMP_ECHOREPLY) {
			break;
		}
	}
}
